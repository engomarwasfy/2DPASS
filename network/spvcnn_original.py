...
class SPVCNN3(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.up1 = nn.ModuleList([
            ...
            nn.Sequential(
                ResidualBlock(cs[4] + cs[2], cs[4], ks=3, stride=1, dilation=1),
                ResidualBlock(cs[4], cs[4], ks=3, stride=1, dilation=1),
            )
        ])
        ...
        self.up2 = nn.ModuleList([
            BasicDeconvolutionBlock(cs[4], cs[5], ks=2, stride=2),
            nn.Sequential(
                ResidualBlock(cs[5] + cs[1], cs[5], ks=3, stride=1, dilation=1),
                ResidualBlock(cs[5], cs[5], ks=3, stride=1, dilation=1),
            )
        ])
        ...
        self.up3 = nn.ModuleList([
            BasicDeconvolutionBlock(cs[5], cs[6], ks=2, stride=2),
            nn.Sequential(
                ResidualBlock(cs[6] + cs[0], cs[6], ks=3, stride=1, dilation=1),
                ResidualBlock(cs[6], cs[6], ks=3, stride=1, dilation=1),
            )
        ])
        ...
        self.classifier = nn.Sequential(nn.Linear(cs[6], kwargs['num_classes']))
    ...
    def forward(self, x):
        ...
        y3 = torchsparse.cat([y3, x1])
        y3 = self.up3[1](y3)
        z3 = voxel_to_point(y3, z2)

        z3.F = z3.F + self.point_transforms[2](z3.F)

        out = self.classifier(z3.F)
        return out


class SPVCNN2(nn.Module):

    def __init__(self, **kwargs):
        super().__init__()

        cr = kwargs.get('cr', 1.0)
        cs = [64, 128, 256, 256, 128]
        cs = [int(cr * x) for x in cs]
...
class SPVCNN2(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.classifier = nn.Sequential(nn.Linear(cs[2], kwargs['num_classes']))
